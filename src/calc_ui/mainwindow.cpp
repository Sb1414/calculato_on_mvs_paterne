...
void MainWindow::trigon_numbers() {
  ...
  if (rez.second == 0) {
    ...
    } else if (!str_form.endsWith('*') && !str_form.endsWith('/') &&
               !str_form.endsWith('+') && !str_form.endsWith('-') &&
               !str_form.endsWith('(')) {
      ui->result->setText(ui->result->text() + "*" + button->text());
    } else {
      ui->result->setText(ui->result->text() + button->text());
...
void MainWindow::on_pushButton_pi_clicked() {
  ...
  if (str == "" || str == '0') {
    ui->result->setText(ui->result->text() + pi);
  } else if (rez.first == 0) {
    ui->result->setText(ui->result->text() + pi);
  } else if (rez.second == 0 &&
             (!std::isdigit(str[j].toLatin1()) && str[j] != '.')) {
    ui->result->setText(ui->result->text() + pi);
...
void MainWindow::on_pushButton_exp_clicked() {
  ...
  size_t j = rez.first - 1;
  if (str == "" || str == '0') {
    ui->result->setText(ui->result->text() + exp);
  } else if (rez.first == 0) {
    ui->result->setText(ui->result->text() + exp);
  } else if (rez.second == 0 &&
             (!std::isdigit(str[j].toLatin1()) && str[j] != '.')) {
...
void MainWindow::on_pushButton_equals_clicked() {
  ...
  if (result.first.size() == 0) {
    ...
    QString result_ = QString::fromLocal8Bit(result.first.c_str());
    ui->result->setText(result_);
  }
}
...
void MainWindow::on_pushButton_graph_clicked() {
  ...
  if (send != "") {
    ...
    if (ui->x2->text().toDouble() >= ui->x1->text().toDouble() ||
      ...
      QMessageBox::warning(this, "Warning", "Неверный интервал x/y",
                           QMessageBox::Ok);
    } else {
...
void MainWindow::on_pushButton_round_bracket_R_clicked() {
  CheckNull();
  if (count_bracket > 0 && checkLastSym()) {
    ui->result->setText(ui->result->text() + ")");
    count_bracket--;
  }
}

bool MainWindow::checkLastSym() {
  QString str_form = ui->result->text();
  if (!str_form.endsWith('*') && !str_form.endsWith('/') &&
      !str_form.endsWith('+') && !str_form.endsWith('-') &&
      !str_form.endsWith('n') && !str_form.endsWith('s') &&
      !str_form.endsWith('t') && !str_form.endsWith('d') &&
      !str_form.endsWith('^') && !str_form.endsWith('s')) {
    return true;
  }
  return false;
}
